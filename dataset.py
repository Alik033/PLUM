...
class Dataset_Load(Dataset):
    def __init__(self, data_path, transform=None):
        ...
        self.filesA, self.filesB = self.get_file_paths(self.data_path, 'LSUI') ## Change it for different datasets.
    ...
    def get_file_paths(self, root, dataset_name):
        if dataset_name=='EUVP':
            ...
            for sd in sub_dirs:
                ...
                filesB = sorted(glob.glob(os.path.join(root, 'reference') + "/*.*"))
        elif dataset_name=='LSUI':
                filesA = sorted(glob.glob(os.path.join(root, 'input') + "/*.*"))
