...
from utilities import calculate_psnr, calculate_ssim, tensor2img
...
def SSIMs_PSNRs(gtr_dir, gen_dir, im_res=(256, 256)):
    ...
    for gtr_path, gen_path in zip(gtr_paths, gen_paths):
        ...
        if (gtr_f==gen_f):
            ...
            ssim = calculate_ssim(np.array(r_im), np.array(g_im))
            ssims.append(ssim)
            # get psnt on L channel (SOTA norm)
            # r_im = r_im.convert("L"); g_im = g_im.convert("L")
            psnr = calculate_psnr(np.array(r_im), np.array(g_im))
            psnrs.append(psnr)
    return np.array(ssims), np.array(psnrs)


# parser.add_argument('--testing_dir_inp', default="/workspace/arijit/alik/LSUI_dwt/output/data/test/input/")
# parser.add_argument('--testing_dir_gt', default="/workspace/arijit/alik/LSUI_dwt/output/data/test/GT/")
...
# # """
# gtr_dir = "/workspace/arijit/alik/LSUI_dwt/output/data/test/GT/"
# # #gtr_dir = "/home/xahid/datasets/released/UFO-120/TEST/hr/"

# # ## generated im paths
# gen_dir = "/workspace/arijit/alik/LSUI_dwt/output/data/test/input/" 
