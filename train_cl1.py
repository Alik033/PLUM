...
# setproctitle("UIE_alik")

def compute_caption_loss(self, gen_image, gt_image):
        # Generate captions
        ...
        gen_caption = generate_caption(gen_image)
        gt_caption = generate_caption(gt_image)

        # Compute caption similarity
        embeddings = sbert_model.encode([gen_caption, gt_caption], convert_to_tensor=True)
        similarity = F.cosine_similarity(embeddings[0].unsqueeze(0), embeddings[1].unsqueeze(0))
        return 1 - similarity.item()  # Caption loss: 1 - similarity
...
if __name__ == '__main__':
	...
	for epoch in range(start_epoch, opt.end_epoch + 1):
		...
		for i_batch, sample_batched in enumerate(dataloader):
			...
			channel_wise_mul = [1.0, 1.5, 2.0]

			batch_mse_loss = 0.0

			for i in range(3):
				batch_mse_loss += (torch.mul(opt.lambda_mse, l1_loss(pred_batch[:,i,:,:], clean_batch[:,i,:,:])) * channel_wise_mul[i])
			...
			print('\r Epoch : ' + str(epoch) + ' | (' + str(i_batch+1) + '/' + str(batches) + ') | mse: ' + str(opt.batch_mse_loss) + ' | vgg: ' + str(opt.batch_vgg_loss), end='', flush=True) 
